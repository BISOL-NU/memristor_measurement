...
def IV_Meas(starting_bias,ending_bias,step,Initial_Sens,delay,sub_plot):
    if not isInitialized(DMM,Init_DMM):
        return
    if(ser.isOpen() == False):
            ser.open()
...
def meas_n(): 
    ...
    delay=float(E7.get())
    save_path =E0.get()
    InV1,Dark_Current1=IV_Meas(starting_bias,ending_bias,step,Initial_Sens,delay,111)
...
tk.Label(subframe_sweep, text="start_Bias (mv)").grid(row=1,column=1,pady=2)
E3 = tk.Entry(subframe_sweep, bd =5)
E3.grid(row=1,column=2,pady=2)
E3.insert(0, "0")

tk.Label(subframe_sweep, text="End Bias (mv)").grid(row=2,column=1,pady=2)
...
tk.Button(subframe_buttons, text ="Dark Current", command = meas_n,bg="red",fg = "blue",
          font=("Arial",10,"bold"),height=3, width=12).grid(row=3,column=0,padx=5,pady=5)
